 ...
 * @author Group15
 */
public class ClusterSkimMeanSimplifyAlgorithm extends TrajectorySummarizationAlgorithm {
    ...
    private List<List<InputPolyLine>> computePolylineGroupings(List<InputPolyLine> inputPolylines, double[][] distances,
            int targetClusterCount) {
        logger.info("computing clusters");
        HierarchicalClustering clusterBuilder = new HierarchicalClustering(distances);
        List<Cluster> lineIdClusters = clusterBuilder.clusterInputPolylines(inputPolylines, targetClusterCount);
        return clustersToPolylineGroups(inputPolylines, lineIdClusters);
    }

    private List<List<InputPolyLine>> clustersToPolylineGroups(List<InputPolyLine> inputPolylines,
    ...
    private List<InputPolyLine> skimToAverageDistance(
        List<InputPolyLine> polyLineGroup,
        double[][] distances
    ) {
        if (polyLineGroup.size() <= 2) {
            return polyLineGroup;
        }
        logger.info("Filtering polyline group");
        logger.info(polyLineGroup.toString());
        double sumOfDistances = 0;
        for (int i=0; i < polyLineGroup.size(); i++) {
            for (int j=0; j < polyLineGroup.size(); j++) {
                InputPolyLine p1 = polyLineGroup.get(i);
                InputPolyLine p2 = polyLineGroup.get(j);
                if (p1.index != p2.index && j>i) {
                    sumOfDistances += distances[p1.index][p2.index];
                    // logger.info(String.format("Distance: %f", distances[p1.index][p2.index]));
                }
            }
        }
        double averageDistance = sumOfDistances / polyLineGroup.size();
        // logger.info(String.format("Average distance: %f", averageDistance));
        List<Integer> filteredIdList = new ArrayList();
        List<InputPolyLine> filteredPolylineGroup = new ArrayList();
        for (int i=0; i < polyLineGroup.size(); i++) {
            for (int j=0; j < polyLineGroup.size(); j++) {
                int originalSize = filteredIdList.size();
                InputPolyLine p1 = polyLineGroup.get(i);
                InputPolyLine p2 = polyLineGroup.get(j);
                logger.info(String.format("Distance: %f, average: %f", distances[p1.index][p2.index], averageDistance));
                if (p1.index != p2.index  && j>i && distances[p1.index][p2.index] <= averageDistance) {
                    if(!filteredIdList.contains(p2.index)){
                        filteredIdList.add(p2.index);
                        filteredPolylineGroup.add(p2);
                    }
                }
                if (filteredIdList.size() > originalSize){
                    // some stuff was added, so the beggining of the pair is also okay
                    filteredIdList.add(p1.index);
                    filteredPolylineGroup.add(p1);
                }
            }
        }
        logger.info(filteredPolylineGroup.toString());
        return filteredPolylineGroup;
    }

    private PolyLine computeMeanPoLyline(List<InputPolyLine> polyLineGroup) {

        logger.info("computing mean polylines");
    ...
    public Output doAlgorithm(Input input) {
        ...
        for (int i = 0; i < input.k && i < groupedPolylines.size(); i++) {
            ...
            List<InputPolyLine> averagedGroup = skimToAverageDistance(outputGroup, distances);
            PolyLine medianPolyline = computeMeanPoLyline(averagedGroup);

            // Step 5: simplify the output?? (Or not?? it might be handy to compute the
            // mean)
